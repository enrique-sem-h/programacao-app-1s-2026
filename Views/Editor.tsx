...
export default function Editor() {
  const [telaAtual, setTelaAtual] = useState<'painel' | 'perfil' | 'gerenciar' | 'editarQualquer'>('painel');

  const TelaPainel = () => (
    <View style={styles.content}>
      <Text style={styles.header}>Painel do Editor</Text>
      
      <TouchableOpacity style={styles.menuItem} onPress={() => setTelaAtual('gerenciar')}>
        <Text style={styles.menuItemText}>Publicar / Despublicar</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.menuItem} onPress={() => setTelaAtual('editarQualquer')}>
        <Text style={styles.menuItemText}>Editar Qualquer Noticia</Text>
      </TouchableOpacity>

      <TouchableOpacity style={[styles.menuItem, {marginTop: 20, borderLeftColor: '#333'}]} onPress={() => setTelaAtual('perfil')}>
  ...
  const TelaPerfil = () => (
    <View style={styles.content}>
      <TouchableOpacity style={styles.backBtn} onPress={() => setTelaAtual('painel')}>
        <Text>← Voltar ao Painel</Text>
      </TouchableOpacity>
      <View style={styles.avatarPlaceholder}><Text style={styles.avatarText}>D</Text></View>
      <Text style={styles.userName}>Diego (Admin)</Text>
      <Text style={{textAlign: 'center', color: '#666'}}>Nivel: Administrador Senior</Text>
    </View>
  );
  ...
  const TelaGerenciar = () => (
    <ScrollView style={styles.content}>
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity style={styles.pubBtn} onPress={() => Alert.alert("Sucesso", "Notícia Publicada!")}>
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity style={[styles.pubBtn, {backgroundColor: '#d0021b'}]} onPress={() => Alert.alert("Sucesso", "Notícia Removida!")}>
    ...
    <View style={styles.container}>
      ...
      {telaAtual === 'editarQualquer' && (
        <View style={styles.content}>
          ...
          <Text style={{color: '#666'}}>Selecione uma noticia para editar</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  menuItem: { backgroundColor: '#fff', padding: 20, borderRadius: 10, elevation: 2, marginBottom: 15, borderLeftWidth: 5, borderLeftColor: '#333' },
  menuItemText: { fontWeight: 'bold', fontSize: 16 },
  avatarPlaceholder: { width: 70, height: 70, borderRadius: 35, backgroundColor: '#333', alignSelf: 'center', justifyContent: 'center', alignItems: 'center', marginBottom: 15 },
