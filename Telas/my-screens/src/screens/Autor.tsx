...
export default function Autor() {
  const [telaAtual, setTelaAtual] = useState<'perfil' | 'lista' | 'formulario'>('perfil');
  ...
  const TelaPerfil = () => (
    <View style={styles.content}>
      ...
      <TouchableOpacity style={styles.menuItem} onPress={() => setTelaAtual('lista')}>
  ...
  const TelaLista = () => (
    <ScrollView style={styles.content}>
      ...
      <TouchableOpacity style={styles.addButton} onPress={() => { setModoEdicao(false); setTelaAtual('formulario'); }}>
        <Text style={styles.buttonText}>+ Nova Noticia</Text>
      </TouchableOpacity>

      <View style={styles.card}>
        <Text style={styles.cardTitle}>AWS chegou na catolica</Text>
        <TouchableOpacity onPress={() => { setModoEdicao(true); setTelaAtual('formulario'); }}>
  ...
  const TelaFormulario = () => (
    <View style={styles.content}>
      <TouchableOpacity style={styles.backBtn} onPress={() => setTelaAtual('lista')}>
        <Text>← Cancelar</Text>
      </TouchableOpacity>
      
      <Text style={styles.header}>{modoEdicao ? 'Editar Noticia' : 'Nova Noticia'}</Text>
      
      <TextInput style={styles.input} placeholder="Titulo da noticia" />
      <TextInput style={[styles.input, { height: 100 }]} placeholder="Conteudo..." multiline />
      
      <TouchableOpacity style={styles.saveButton} onPress={() => {
        Alert.alert("Sucesso", modoEdicao ? "Atualizado!" : "Criado!");
        setTelaAtual('lista');
      }}>
        <Text style={styles.buttonText}>{modoEdicao ? 'Salvar Alterações' : 'Publicar'}</Text>
      </TouchableOpacity>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  avatarPlaceholder: { width: 70, height: 70, borderRadius: 35, backgroundColor: '#4a90e2', alignSelf: 'center', justifyContent: 'center', alignItems: 'center', marginBottom: 15 },
  avatarText: { color: '#fff', fontSize: 24, fontWeight: 'bold' },
  profileName: { fontSize: 18, fontWeight: 'bold', textAlign: 'center', marginBottom: 30 },
  ...
  input: { backgroundColor: '#fff', borderWeight: 1, borderColor: '#ddd', padding: 12, borderRadius: 8, marginBottom: 15 }
});